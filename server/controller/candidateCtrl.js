    ...
    try {
      ...
      if (userEmail.flag == 0) {
        ...
        if (draft == 0) {
          ...
          pdfDoc.moveDown();
          if (academics == 1) {
            const record1 = await Degree.findOne({
              userId: req.user.id,
              count: 0,
            });
            const table = {
              ...
              rows: [
                [
                  `1`,
                  `${record1.degreeName}`,
                  `${record1.universityName}`,
                  `${record1.yearOfPassing}`,
                  `${record1.score}`,
                ],
              ],
              options: {
            ...
            const record2 = await Degree.findOne({
              userId: req.user.id,
              count: 2,
            });
            const record3 = await Degree.findOne({
              userId: req.user.id,
              count: 3,
            });
            const table = {
              ...
              rows: [
                ...
                [
                  `2`,
                  `${record2.degreeName}`,
                  `${record2.universityName}`,
                  `${record2.yearOfPassing}`,
                  `${record2.score}`,
                ],
                [
                  `3`,
                  `${record3.degreeName}`,
                  `${record3.universityName}`,
                  `${record3.yearOfPassing}`,
                  `${record3.score}`,
                ],
              ],
              options: {
          ...
          if (netExam == 1) {
            const rec1 = await Academic.findOne({ userId: req.user._id });
            const table1 = {
          ...
          if (academics != 1) {
            pdfDoc.addPage();
            const proofImg2 = await fetchImage(`${req.body.proof2}`);
            pdfDoc
              .image(proofImg2, 60, 100, {
                // width: 500,
              })
              .stroke();

            pdfDoc.addPage();
            const proofImg3 = await fetchImage(`${req.body.proof3}`);
            pdfDoc
              .image(proofImg3, 60, 100, {
                // width: 500,
              })
              .stroke();
          }
          ...
          if (netExam == 1) {
            ...
            pdfDoc
              .font("Times-Roman", "Bold")
              .fontSize(14)
              .text("Details of NET EXAM:", { align: "left" });
            // console.log();
            const proofI = await fetchImage(`${req.body.certificate}`);
            pdfDoc
              .image(proofI, 60, 100, {
                // width: 500,
              })
  ...
  submitApplication: async (req, res) => {
    try {
      ...
      if (academics == 1) {
        ...
        const checkNew = await Degree.findOne({
          userId: req.user.id,
          count: 1,
        });
      ...
      if (draft == 0) {
        ...
        if (netExam == 1) {
          const rec1 = await Academic.findOne({ userId: req.user.id });
          const table1 = {
            headers: ["#", "Name as in Exam", "Score", "Validity"],
            rows: [
              [
